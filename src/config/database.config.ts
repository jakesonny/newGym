...
import {
  appendPgSearchPath,
  isSslRequiredHost,
} from "../common/utils/pg-connection-url";
...
export const getDatabaseConfig = (
  ...
  const commonConfig = {
    ...
    logging:
      configService.get<string>("DB_LOGGING") === "true" || isDevelopment,
    extra: {
  ...
  if (databaseUrl) {
    return {
      ...
      ssl: isSslRequiredHost(databaseUrl)
  ...
  const isProduction = isSslRequiredHost(host);
