...
export class AuthController {
  ...
  @ApiOperation({
    summary: "로그아웃",
    description: "로그아웃합니다. DB에서 refreshToken을 삭제합니다.",
  })
  ...
  @ApiOperation({
    summary: "토큰 갱신",
    description:
      "refreshToken으로 새로운 accessToken과 refreshToken을 발급받습니다.",
  })
  ...
  async kakaoLogin(@Request() req) {
    ...
    this.logger.debug(
      `요청 IP: ${req.ip}, User-Agent: ${req.headers["user-agent"]}`,
    );
  ...
  @ApiOperation({
    summary: "카카오 로그인 콜백",
    description:
      "카카오 인증 후 콜백을 처리하고 JWT 토큰을 실어서 앱의 딥링크로 리다이렉트합니다.",
  })
  @ApiResponse({
    status: 302,
    description: "카카오 로그인 성공 - 앱 딥링크로 리다이렉트",
  })
  @ApiResponse({ status: 401, description: "인증 실패" })
  async kakaoCallback(
    @Request() req,
    ...
    try {
      ...
      if (!accessToken) {
        ...
        this.logger.error(
          `req.user 구조: ${JSON.stringify(Object.keys(req.user))}`,
        );
        throw new Error("액세스 토큰을 생성할 수 없습니다.");
      }

      this.logger.log(
        `카카오 로그인 성공: 사용자 ${req.user.user?.email || req.user.user?.id}`,
      );
      ...
      this.logger.debug(
        `딥링크 리다이렉트: ${redirectUrl.replace(accessToken, "***")}`,
      );
      ...
      this.logger.error(
        `요청 정보: ${JSON.stringify({ query, user: req.user ? Object.keys(req.user) : null })}`,
      );
  ...
  @ApiOperation({
    summary: "본인 정보 수정",
    description:
      "현재 로그인한 사용자의 정보를 수정합니다. (이름, 이메일, 비밀번호)",
  })
  ...
  @ApiResponse({
    status: 400,
    description:
      "이미 등록된 이메일 또는 소셜 로그인 사용자의 비밀번호 변경 시도",
  })
  ...
  @ApiOperation({
    ...
    description:
      "관리자가 다른 사용자의 정보를 수정합니다. (이름, 이메일, 비밀번호, 역할)",
  })
  ...
  @ApiBearerAuth("JWT-auth")
  @ApiOperation({
    summary: "승인 대기 TRAINER 목록",
    description: "ADMIN 승인을 기다리는 TRAINER 목록을 조회합니다. (ADMIN만)",
  })
  ...
  async getPendingTrainers() {
    ...
    // 비밀번호 제외하고 반환
    const trainers = pendingTrainers.map(
      ({ password, refreshToken, ...trainer }) => trainer,
    );
  ...
  @ApiBearerAuth("JWT-auth")
  @ApiOperation({
    summary: "전체 TRAINER 목록 조회",
    description:
      "모든 TRAINER 목록을 조회합니다. (승인됨, 대기중 모두 포함, ADMIN만)",
  })
  ...
  @ApiOperation({
    summary: "TRAINER 승인",
    description: "승인 대기 중인 TRAINER를 승인합니다. (ADMIN만)",
  })
  ...
  async approveTrainer(@Param("id") id: string, @Request() req) {
    const approvedTrainer = await this.authService.approveTrainer(
      id,
      req.user.id,
    );
  ...
  @ApiBearerAuth("JWT-auth")
  @ApiOperation({
    summary: "TRAINER 승인 취소",
    description: "이미 승인된 TRAINER의 승인을 취소합니다. (ADMIN만)",
  })
  @ApiParam({ name: "id", description: "TRAINER ID (UUID)", type: "string" })
  ...
  @ApiResponse({
    status: 400,
    description: "이미 승인되지 않은 TRAINER입니다.",
  })
  async disapproveTrainer(@Param("id") id: string, @Request() req) {
    const disapprovedTrainer = await this.authService.disapproveTrainer(
      id,
      req.user.id,
    );
  ...
  @ApiOperation({
    summary: "TRAINER 거부",
    description:
      "승인된 TRAINER를 거부합니다. (isApproved를 false로 변경, 계정은 삭제하지 않음, ADMIN만)",
  })
  @ApiParam({ name: "id", description: "TRAINER ID (UUID)", type: "string" })
  ...
  async rejectTrainer(@Param("id") id: string, @Request() req) {
    const rejectedTrainer = await this.authService.rejectTrainer(
      id,
      req.user.id,
    );
