-- ============================================
-- 데이터베이스 클론 스크립트 Part 1: 스키마 생성
-- DBeaver에서 실행용
-- ============================================
-- 사용법: 이 파일을 먼저 실행하여 스키마를 생성합니다.
-- 주의: 모든 기존 데이터가 삭제됩니다!
...
-- 13. exercises 테이블 (마이그레이션 반영: name nullable, body_part 추가, name_en UNIQUE)
CREATE TABLE exercises (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    name VARCHAR(255), -- nullable (마이그레이션 반영)
    name_en VARCHAR(255) NOT NULL UNIQUE, -- UNIQUE 제약조건 추가 (마이그레이션 반영)
    category exercise_category NOT NULL DEFAULT 'FULL_BODY',
    body_part VARCHAR(50), -- 마이그레이션으로 추가된 컬럼
...
-- 14. strength_standards 테이블 (마이그레이션 반영: standard_type, age_min, age_max 추가)
CREATE TABLE strength_standards (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    exercise_id UUID NOT NULL REFERENCES exercises(id) ON DELETE CASCADE,
    standard_type VARCHAR(20) NOT NULL DEFAULT 'BODYWEIGHT', -- 마이그레이션으로 추가
    bodyweight_min FLOAT, -- nullable: 나이별 데이터의 경우 NULL 가능
    bodyweight_max FLOAT, -- nullable: 나이별 데이터의 경우 NULL 가능
    age_min INTEGER, -- 마이그레이션으로 추가 (nullable)
    age_max INTEGER, -- 마이그레이션으로 추가 (nullable)
    gender gender NOT NULL,
    level strength_level NOT NULL,
    weight_kg FLOAT NOT NULL,
    created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    -- 체중별 데이터는 bodyweight_min/max가 필수, 나이별 데이터는 age_min/max가 필수
    CONSTRAINT chk_strength_standards_check CHECK (
        (standard_type = 'BODYWEIGHT' AND bodyweight_min IS NOT NULL AND bodyweight_max IS NOT NULL) OR
        (standard_type = 'AGE' AND age_min IS NOT NULL AND age_max IS NOT NULL)
    )
);
...
-- exercises 인덱스 (body_part 인덱스 추가 - 마이그레이션 반영)
...
-- strength_standards 인덱스 (마이그레이션 반영: standard_type, age 인덱스 추가)
...
CREATE INDEX idx_strength_standards_standard_type ON strength_standards(standard_type);
CREATE INDEX idx_strength_standards_age ON strength_standards(age_min, age_max);
CREATE INDEX idx_strength_standards_lookup ON strength_standards(exercise_id, gender, level, standard_type, bodyweight_min, bodyweight_max, age_min, age_max);
...
-- Part 1 완료 메시지
-- ============================================
SELECT 'Part 1: Schema created successfully! Now run dbclone_02_exercises.sql' AS message;
