...
export class ScoreCalculator {
  ...
  private calculateTotalScore(categoryScores: CategoryScores): number {
    ...
    if (
      categoryScores.strengthScore !== null &&
      categoryScores.strengthScore !== undefined
    ) {
      weightedSum += categoryScores.strengthScore * weights.strength;
      totalWeight += weights.strength;
    }
    if (
      categoryScores.cardioScore !== null &&
      categoryScores.cardioScore !== undefined
    ) {
      weightedSum += categoryScores.cardioScore * weights.cardio;
      totalWeight += weights.cardio;
    }
    if (
      categoryScores.enduranceScore !== null &&
      categoryScores.enduranceScore !== undefined
    ) {
      weightedSum += categoryScores.enduranceScore * weights.endurance;
      totalWeight += weights.endurance;
    }
    if (
      categoryScores.flexibilityScore !== null &&
      categoryScores.flexibilityScore !== undefined
    ) {
      weightedSum += categoryScores.flexibilityScore * weights.flexibility;
      totalWeight += weights.flexibility;
    }
    if (
      categoryScores.bodyScore !== null &&
      categoryScores.bodyScore !== undefined
    ) {
      weightedSum += categoryScores.bodyScore * weights.body;
      totalWeight += weights.body;
    }
    if (
      categoryScores.stabilityScore !== null &&
      categoryScores.stabilityScore !== undefined
    ) {
  ...
  /**
   * 평가 점수 계산 및 스냅샷 저장
   * @param assessedAt 평가 실시 시각. 시드/과거 데이터 입력을 위해 클라이언트가
   *   지정한 값을 그대로 반영해야 한다(생략 시에만 현재 시각으로 대체).
   *   이 값이 서버 실행 시각으로 고정되면 주간 비교 등 시계열 통계가 항상 0이 된다.
   */
