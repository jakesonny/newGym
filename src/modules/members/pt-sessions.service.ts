...
export class PTSessionsService {
  ...
  async findAll(memberId: string): Promise<{
    ...
    await RepositoryHelper.ensureMemberExists(
  ...
  async create(
    ...
    try {
      ...
      if (createDto.membershipId && this.hasMeasurement(createDto)) {
        await this.updateMembershipTrend(
          queryRunner,
  ...
  private async updateMembershipTrend(
    ...
    // 주의: 이 시점에 방금 생성된 세션은 이미 같은 트랜잭션 내에서 저장되어
    // recentSessions에 포함되어 있다. 따라서 currentValue를 별도로 다시
    // push하면 마지막 측정값이 중복 반영되어 shortTermDelta가 항상 0이 된다.
    const recentSessions = await queryRunner.manager.find(PTSession, {
      where: { membershipId },
      order: { sessionDate: "ASC" },
    });

    // 측정값 배열 생성 (방금 저장된 세션 포함, 중복 push 금지)
    ...
    // Membership 업데이트
    // currentProgress 컬럼은 int이므로 ProgressCalculator가 반환하는 소수점 1자리
    // 값(예: 62.3)을 그대로 저장하면 DB 저장이 실패한다. 저장 직전에 반올림한다.
  ...
  async update(
    ...
    EntityUpdateHelper.updateFieldsWithDateConversion(session, updateDto, [
      "sessionDate",
    ]);
  ...
  private async createOrUpdateMilestone(
    ...
    const isBlockEndWeek =
      currentWeek === 4 ||
      currentWeek === 8 ||
      currentWeek === membership.durationWeeks;
    if (!isBlockEndWeek) {
      return;
    }
    ...
    const blocks = ProgressCalculator.generateMilestoneBlocks(
      startDate,
      membership.durationWeeks as 4 | 8 | 12,
    );
    ...
    // 기존 마일스톤 조회 (같은 블록)
    const existingMilestone = await queryRunner.manager.findOne(
      ProgramMilestone,
      {
        where: {
          membershipId,
    ...
    // 진행률 계산
    // progress_at_milestone 컬럼은 int이므로 소수점 결과(예: 62.3)를 그대로
    // 저장하면 실패한다. 저장 직전에 반올림한다.
    let progressAtMilestone: number | null = null;
    if (
      membership.startValue != null &&
      membership.targetValue != null &&
      measuredValue != null
    ) {
      progressAtMilestone = Math.round(
    ...
    if (existingMilestone) {
      ...
      await queryRunner.manager.update(ProgramMilestone, existingMilestone.id, {
        ...
        achievedAt:
          progressAtMilestone != null && progressAtMilestone >= 100
            ? new Date()
            : null,
      });
      this.logger.log(
        `마일스톤 업데이트: Membership ${membershipId}, Block ${blockNumber}, Week ${currentWeek}`,
      );
    } else {
      // 새 마일스톤 생성
      ...
      this.logger.log(
        `마일스톤 생성: Membership ${membershipId}, Block ${blockNumber}, Week ${currentWeek}`,
      );
    }
  }
}
